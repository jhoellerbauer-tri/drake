...
// for Expression.
// We only need one test case because if the specialization doesn't exist, this
// would results in a compile error.
...
TEST_F(SymbolicExpressionTest, EigenNotEqualStrict) {
  ASSERT_TRUE(Eigen::numext::not_equal_strict(c3_, c4_));
}
